            ...
            Row(
              children: [
                if (CacheHelper.getData(key: CacheKeys.token) != null && 
                    storeDataModel.vendor!.id.toString() == userId)
                ...
                if (CacheHelper.getData(key: CacheKeys.token) != null && storeDataModel.vendor!.id.toString() != userId)
                  BlocConsumer<ServicesCubit, ServicesState>(
                    listener: (context, state) {
                    ...
                    builder: (context, state) {
                      ...
                      return CustomElevatedButton(
                        ...
                        storeDataModel.vendor!.id!.toString()] ?? false
                            ? LocaleKeys.unFollow.tr()
                            : LocaleKeys.follow.tr(),
                        onPressed: () {
                          if (!cubit.followedVendors.containsKey(storeDataModel.vendor?.id?.toString()) ||
                              !(cubit.followedVendors[storeDataModel.vendor?.id?.toString()] ?? false)) {
